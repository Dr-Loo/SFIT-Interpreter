# Transmon_SFIT_Encoder_T2_feedback_v4.py
# Finite-difference gradients + projector rebuild per trial (no complex-step).
import numpy as np
...
def dk_budget_and_gap(d0c: np.ndarray, d1c: np.ndarray, M0c: np.ndarray, M1c: np.ndarray, M2c: np.ndarray,
    ...
    dk = float(norm(Em, 'fro')) / max(gap, 1e-15)
    return gap, dk
...
def eval_loss(bx: float, by: float, *,
              ...
              Nx_f: int, Ny_f: int,
              freeze_projector: bool) -> Tuple[float, np.ndarray, float]:
    """
    Build fine metric for (bx,by), (optionally) rebuild projector, push forward + noise,
    pull back, and compute loss to target alpha (embedded in Bc,M1c via cycle_weights).
    ...
    # Projector: if you freeze it, you won't see gradients. Rebuild for true dependence.
    Pif, Bf = transported_projector(M1f, P, Bc) if not freeze_projector else transported_projector(M1f, P, Bc)
    ...
    # target from v_c_clean (computed once outside); we rebuild it here for locality
    rho_t, _, alpha_t = cycle_weights(M1c, Bc, R @ (transported_projector(apply_metric_bias(M1f, 1.0, 1.0, Nx_f, Ny_f), P, Bc)[0] @ v_c_clean))
    # Loss: match alpha & rho (weights are tunable)
...
def run_feedback(
    ...
    freeze_projector: bool = False,   # IMPORTANT: must be False to see gradients
    steps: int = 20,
    lr: float = 0.5,
    seed: int = 0,
    fd_h: float = 1e-3,
    ...
    for it in range(steps):
        # Common noise seed per-iteration for fair finite differences
        eta_seed = 1000 + it

        # Current diagnostics (also build current fine ops for gap/DK report)
        ...
        loss, alpha_r, S_r = eval_loss(bias_x, bias_y,
        ...
        # Finite-difference gradient (with same seed)
        gx, gy = finite_diff_grad(bias_x, bias_y, fd_h,
                                  P=P, R=R, Bc=Bc, M1c=M1c,
                                  v_c_clean=v_f_clean,
                                  eta_seed=eta_seed,
                                  noise=noise,
...
if __name__ == "__main__":
    run_feedback(
        ...
        freeze_projector=False,     # <— allow loss to depend on (bias_x, bias_y)
        steps=20, lr=0.5, seed=0,
        fd_h=1e-3,
    )
